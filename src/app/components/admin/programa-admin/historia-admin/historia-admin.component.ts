...
@Component({
  selector: "app-historia-admin",
  templateUrl: "./historia-admin.component.html",
  styleUrls: ["./historia-admin.component.css"],
})
export class HistoriaAdminComponent implements OnInit {
  closeResult: string;
  war: any;
  modalReference: any;
  error = false;
  ...
  constructor(
    ...
    private historiaServices: HistoriaService,
  ...
  guardar() {
    if (
      this.historia.reseniaHistorica !== this.war ||
      this.historia.reseniaHistorica !== this.war
    ) {
      this.error = false;
      this.modalReference.close();
      if (this.id === "nuevo") {
        this.historiaServices.nuevoHistoria(this.historia).subscribe(
          (data) => {
            this.showSuccess();
        ...
        this.modalReference.close();
        this.historiaServices
          .actualizarHistoria(this.historia, this.id)
          .subscribe(
            (data) => {
              this.showInfo();
              this.router.navigate(["/historia"]);
