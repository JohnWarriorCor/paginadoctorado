...
@Component({
  selector: "app-grupos-investigacion-admin",
  templateUrl: "./grupos-investigacion-admin.component.html",
  styleUrls: ["./grupos-investigacion-admin.component.css"],
  providers: [DatePipe],
})
export class GruposInvestigacionAdminComponent implements OnInit {
  today = new Date();
  fecha: any;
  closeResult: string;
  defaultImgUrl: any;
  ...
  guardar() {
    if (
      this.grupoInvestigacion.fieldArray[0] !== this.war ||
      this.grupoInvestigacion.fieldArray[0] !== this.war
    ) {
      this.error = false;
      ...
      if (this.id === "nuevo") {
        this.grupoInvestigacionService
          .nuevoGrupo(this.grupoInvestigacion)
          .subscribe(
            (data) => {
        ...
        this.modalReference.close();
        this.grupoInvestigacionService
          .actualizarGrupo(this.grupoInvestigacion, this.id)
          .subscribe(
            (data) => {
              this.router.navigate(["/gruposInvestigacion"]);
              this.modalReference.close();
