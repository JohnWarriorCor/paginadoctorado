...
@Component({
  selector: "app-articulo-docentes-admin",
  templateUrl: "./articulo-docentes-admin.component.html",
  styleUrls: ["./articulo-docentes-admin.component.css"],
  providers: [DatePipe],
})
export class ArticuloDocentesAdminComponent implements OnInit {
  today = new Date();
  fecha: any;
  closeResult: string;
  defaultImgUrl: any;
    ...
    this.activatedRoute.params.subscribe((parametros) => {
      ...
      if (this.id !== "nuevo") {
        this.articulosProService
          ...
          .subscribe((articulosPro) => (this.articulosPro = articulosPro));
      }
    });
  }
  showSuccess() {
  ...
  guardar() {
    if (
      this.articulosPro.nombreArticulo !== this.war ||
      this.articulosPro.nombreArticulo !== this.war
    ) {
      this.error = false;
      ...
      if (this.id === "nuevo") {
        this.articulosProService
          .nuevoArticuloProfesor(this.articulosPro)
          .subscribe(
            (data) => {
              this.showSuccess();
        ...
        this.modalReference.close();
        this.articulosProService
          .actualizarArticuloProfesor(this.articulosPro, this.id)
          .subscribe(
            (data) => {
              this.showInfo();
              this.router.navigate(["/articulosProfesores"]);
