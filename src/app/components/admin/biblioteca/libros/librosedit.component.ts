...
export class LibroseditComponent implements OnInit {
  ...
  constructor(public datepipe: DatePipe, private modalService: NgbModal, private planEstudioService: BibliotecaService, private router: Router, private activatedRoute: ActivatedRoute) {
    this.activatedRoute.params.subscribe( parametros => {
      ...
      if ( this.id !== 'nuevo' ) {
        this.planEstudioService.getLibro( this.id ).subscribe(libro => this.libro = libro);
      }
    });
  }
  ...
  guardar() {
    if ( this.libro.fieldArray[0] !== this.war ||  this.libro.fieldArray[0] !== this.war ) {
      this.error = false;
      this.modalReference.close();
      if ( this.id === 'nuevo' ) {
        this.planEstudioService.nuevoLibro( this.libro ).subscribe(data => {
        ...
        this.modalReference.close();
        this.planEstudioService.actualizarLibro( this.libro, this.id ).subscribe(data => {
          this.router.navigate(['/biblioteca']);
          this.modalReference.close();
        },
        error => console.error(error));
