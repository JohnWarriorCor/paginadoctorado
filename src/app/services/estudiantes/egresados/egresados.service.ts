...
export class EgresadosService {
  egresadoRegistroURL =
    "https://doctoradocienciasdelasaludusco.firebaseio.com/listadoEgresados.json";
  egresadoURL =
    "https://doctoradocienciasdelasaludusco.firebaseio.com/listadoEgresados/";
  ...
  nuevoEgresado(listado: Egresados) {
    ...
    return this.http.post(this.egresadoRegistroURL, body, { headers }).pipe(
      map((res) => {
  ...
  getArticuloEgresado(key$: string, idx$: string) {
    const url = `${
      this.egresadoURL
    }/${key$}/${"fieldArrayArticulos"}/${idx$}.json`;
    return this.http.get(url).pipe(map((res) => res.json()));
  }
  getEgresados() {
    return this.http
      .get(this.egresadoRegistroURL)
